...
from src.upsampling_pytorch import UpsampleOutConvTrainer, get_cm
...
net = UpsampleOutConvTrainer(n_channels = 20, 
                      bias = True,
...
for i in range(20): 
    plt.imshow(net.conv(batch[:1]).reshape((20,7,7))[i].detach().numpy())
    plt.show()            
...
using_cuda    = "Network using cuda?: " + str(next(net.parameters()).is_cuda)
print(gpu_available)
print(using_cuda)
...
for epoch in range(100000):
    ...
    #get output        
    output = net(batch.cuda())
    
    #calculate loss (target is same as input)
    loss = criterion2(output[0], batch.cuda())
    ...
    if (epoch +1) % 100 == 0:
        ...
        if (epoch +1) % 1000 == 0:
            ...
            out = net(batch[i-1:i].cuda())[0].cpu().detach().numpy().reshape(8,8)
